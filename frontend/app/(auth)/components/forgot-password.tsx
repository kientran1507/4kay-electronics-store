...
export default function ForgotPassword() {
  ...
  return <CustomerAuthShell title="Reset your password" subtitle="Enter your email and we’ll help you reset your password."><form onSubmit={(event) => { event.preventDefault(); setMessage("Password reset is not available yet. Please contact support."); }} className="space-y-4">{message && <p className="rounded-xl bg-[#fff4e6] p-3 text-sm text-[#9a5a08]">{message}</p>}<Field label="Email" type="email" value={email} onChange={setEmail} /><button className="h-11 w-full rounded-xl bg-[#c87916] font-semibold text-white">Continue</button><p className="text-center text-sm"><Link href="/customer-sign-in" className="font-semibold text-[#c87916]">Back to Sign in</Link></p></form></CustomerAuthShell>;
}
